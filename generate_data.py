...
baro_file, mpu_file, gps_file = (open("baro.csv", "w"), open("mpu.csv", "w"), open("gps.csv", "w"))
...
v0 = upward_vel[i]
h0 = alt[i]
while alt[i] < 1600: # 1600 meters = ~5000 ft
    i += 1
...
# third stage of trajectory: cutdown & descent
i0 = i
t0 = time[i]
v0 = upward_vel[i]
h0 = alt[i]
print(f"i0: {i0}")
for i in range(i + 1, 20001):
    t = time[i]

    upward_accel[i] = -10
    upward_vel[i] = v0 - 10 * (t - t0)
    alt[i] = -0.5 * 10 * (t - t0) ** 2 + v0 * (t - t0) + h0
    ...
    if alt[i] < 0:
        print(f"breaking: i: {i}, alt: {alt[i]}")
        i_f = i
        break
else:
    i_f = 20001
